export default function M3View({ content }) {
  return (
    <div className="flex flex-col h-full bg-[#0B0C10] text-[#C5C6C7] rounded-b-2xl">
      <div className="px-6 py-4 border-b border-white/5 bg-brand-accentDark/10">
        <h3 className="font-semibold text-brand-light flex items-center gap-2">
          Dependency Matrix
        </h3>
        <p className="text-sm text-brand-light/60 mt-1">Shows how internal application modules interact</p>
      </div>
      <div className="flex-1 p-6 overflow-auto custom-scrollbar">
        {content ? (
          ...
          <div className="flex items-center justify-center h-full text-brand-light/30">
            No dependency relations detected
          </div>
        )}
      </div>
    </div>
